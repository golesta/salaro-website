// The hero visual: a live, programmatic diagram of how Salaro builds.
// Four roles (Architect, Researcher, Developer, Reviewer) pass tokens of work
// across a horizontal axis toward "Shipped". Human-judgment node sits above,
// occasionally pulses to override. No images, no video — pure SVG + state.
...
function AgentFactoryDiagram({ width = 1200, height = 420 }) {
  ...
  React.useEffect(() => {
    ...
    const tick = (now) => {
      ...
      // human-judgment pulse roughly every 4s
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: 'auto', display: 'block' }}>
      {/* Conveyor line */}
      <line
        x1={width * 0.04} y1={yMid}
        x2={width * 0.96} y2={yMid}
        stroke="var(--line)" strokeWidth="0.8"
      />
      {/* Tick marks between roles */}
        ...
        return (
          <g key={r.id}>
            <circle
              cx={x} cy={yMid}
              r={isShipped ? 9 : 6}
              fill={isShipped ? 'var(--accent)' : 'var(--bg)'}
              stroke={isShipped ? 'var(--accent)' : 'var(--ink-strong)'}
              strokeWidth="1.2"
            />
            {isShipped && (
              <circle cx={x} cy={yMid} r={18} fill="var(--accent)" opacity="0.12" />
            )}
            <text
              x={x} y={yMid + 32}
              textAnchor="middle"
      ...
      <g>
        ...
        />
        {pulse && (
          <circle cx={width * 0.5} cy={yHuman} r="22" fill="var(--ink-strong)" opacity="0.08" />
        )}
        <text
          x={width * 0.5} y={yHuman - 22}
          textAnchor="middle"
          fontFamily="var(--font-mono)"
          fontSize="10"
          letterSpacing="2"
      ...
      {tokens.map(t => {
        ...
        return (
          <g key={t.id}>
            <circle
              cx={x} cy={y} r="3"
              fill={isFlagged ? 'var(--accent)' : 'var(--ink-strong)'}
            />
            {isFlagged && (
      ...
      {/* Bottom apparatus label */}
      <text
        x={width * 0.04} y={height - 12}
        fontFamily="var(--font-mono)"
        fontSize="10"
        letterSpacing="2.4"
        fill="var(--ink-faint)"
...
window.AgentFactoryDiagram = AgentFactoryDiagram;
