...
export default function AgentFactory({ width = 1200, height = 420 }: { width?: number; height?: number }) {
      ...
      {tokens.map((t) => {
        ...
        return (
          <g key={t.id}>
            <circle
              cx={x} cy={y} r="3"
              fill={t.kind === "flagged" ? "var(--accent)" : "var(--ink-strong)"}
            />
            {t.kind === "flagged" && (
